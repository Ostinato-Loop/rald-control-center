...
ai.get("/api/ai-providers", async (c) => {
  ...
  const { data, error } = await db.from("ai_providers").select("*").order("routing_priority");
...
ai.post("/api/ai-providers", async (c) => {
  ...
  const { data, error } = await db.from("ai_providers").insert(insert).select().single();
  if (error) return c.json({ error: error.message }, 500);
...
ai.patch("/api/ai-providers/:id", async (c) => {
  ...
  const { data, error } = await db.from("ai_providers").update(update).eq("id", c.req.param("id")).select().single();
  if (error) return c.json({ error: error.message }, 500);
...
ai.delete("/api/ai-providers/:id", async (c) => {
  ...
  if (!payload) return c.json({ error: "Unauthorized" }, 401);
  const db = getSupabase(c.env);
  const { error } = await db.from("ai_providers").delete().eq("id", c.req.param("id"));
  if (error) return c.json({ error: error.message }, 500);
