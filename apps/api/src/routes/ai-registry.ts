...
registry.get("/api/ai-registry", async (c) => {
  ...
  const { data, error } = await db.from("ai_models").select("*, ai_providers(name, provider_type)").order("routing_priority");
...
registry.post("/api/ai-registry", async (c) => {
  ...
  const db = getSupabase(c.env);
  const { data, error } = await db.from("ai_models").insert(body).select().single();
  if (error) return c.json({ error: error.message }, 500);
...
registry.patch("/api/ai-registry/:id", async (c) => {
  ...
  if (!payload) return c.json({ error: "Unauthorized" }, 401);
  const db = getSupabase(c.env);
  const { data, error } = await db.from("ai_models").update(await c.req.json()).eq("id", c.req.param("id")).select().single();
  if (error) return c.json({ error: error.message }, 500);
...
registry.delete("/api/ai-registry/:id", async (c) => {
  ...
  if (!payload) return c.json({ error: "Unauthorized" }, 401);
  const db = getSupabase(c.env);
  const { error } = await db.from("ai_models").delete().eq("id", c.req.param("id"));
  if (error) return c.json({ error: error.message }, 500);
