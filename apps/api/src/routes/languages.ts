...
langs.get("/api/languages", async (c) => {
  ...
  const { data, error } = await db.from("rald_cc_language_packs").select("*").order("language_name");
...
langs.post("/api/languages", async (c) => {
  ...
  const db = getSupabase(c.env);
  const { data, error } = await db.from("rald_cc_language_packs").insert(body).select().single();
  if (error) return c.json({ error: error.message }, 500);
...
langs.patch("/api/languages/:id", async (c) => {
  ...
  if (!payload) return c.json({ error: "Unauthorized" }, 401);
  const db = getSupabase(c.env);
  const { data, error } = await db.from("rald_cc_language_packs").update(await c.req.json()).eq("id", c.req.param("id")).select().single();
  if (error) return c.json({ error: error.message }, 500);
