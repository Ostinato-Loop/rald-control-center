...
const wizmacNav = [
  { label: "Service Health",    href: "/wizmac",    icon: Activity },
  { label: "Knowledge Core",    href: "/knowledge", icon: Database },
];
...
export function Sidebar() {
  ...
  function NavItem({ label, href, icon: Icon }: { label: string; href: string; icon: typeof Activity }) {
    ...
    <aside className="w-60 flex-shrink-0 flex flex-col border-r border-[var(--border)]" style={{ background: "var(--surface)" }}>
      ...
      <nav className="flex-1 py-3 overflow-y-auto">
        ...
        <div className="px-4 pt-4 pb-1">
          <div className="text-[10px] font-mono uppercase tracking-widest text-[var(--muted)] flex items-center gap-2">
            <span className="text-base">🧠</span> WIZMAC
          </div>
        </div>
